<template>
  <div class="login-box">
    <div class="login-logo">
      ...
      <img src="~@/assets/images/system/logo.png" width="200" />
    </div>
    <a-form layout="horizontal" :model="state.formInline" @submit.prevent="handleSubmit">
      ...
      <a-form-item>
        <a-input v-model:value="state.formInline.password" size="large" type="password" placeholder="password">
          <template #prefix>
      ...
      <a-form-item>
        <a-input v-model:value="state.formInline.verifyCode" placeholder="captcha" :maxlength="4" size="large">
          <template #prefix>
            <SafetyOutlined />
          </template>
          <template #suffix>
...
const setCaptcha = async () => {
  const { id, img } = await getImageCaptchaApi({ width: 100, height: 50 });
  state.captcha = img;
...
const handleSubmit = async () => {
  ...
  if (username.trim() == '' || password.trim() == '') {
    return globalProperties.$message.warning('用户名或密码不能为空！');
  }
  if (!verifyCode) {
    return globalProperties.$message.warning('请输入验证码！');
  }
  globalProperties.$message.loading('登录中...', 0);
  state.loading = true;
  ...
  try {
    await loginApi(state.formInline);
  ...
  state.loading = false;
  globalProperties.$message.destroy();
};
</script>
...
.login-box {
  ...
  height: 100vh;
  padding-top: 240px;
  background: url('~@/assets/images/system/login.svg');
  background-size: 100%;
